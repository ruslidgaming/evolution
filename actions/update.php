...
if(isset($_POST)){
    ...
    if(!empty($_FILES['productImg']['name'])) {
        if(empty($_FILES['productImg']['name'])) {
            ...
            header('Location: ../index.php?page=admin&admin=edit');
    ...
    if(!empty($_SESSION['errors'])) {
        header("Location: ../index.php?page=admin&admin=edit&id=$id");
        die();
    }

    if(isset($fileName) && !empty($fileName)){
        $sql = "UPDATE `stickers` SET `name` = '$name', `description` = '$description', `img` = '$fileName' WHERE `stickers`.`id` = '$id'";
        
    }
    else{
        $sql = "UPDATE `stickers` SET `name` = '$name', `description` = '$description' WHERE `stickers`.`id` = '$id'";
    }
